...
fn main() {
    ...
    for update in updates {
        ...
        for n1 in &update {
            set.insert(*n1);
            let rule = rules.entry(*n1).or_insert(vec![]);
            if rule.into_iter().any(|n2| set.contains(n2)) {
                valid = false;
                let mut first_idx = 0;
                for (i, n) in new_update.clone().into_iter().enumerate() {
                    if rule.into_iter().any(|n2| n2 == n) {
                        first_idx = i;
                        break;
                    }
                }

                new_update.insert(first_idx, n1);
            } else {
                new_update.push(n1);
            }
        }
