...
fn main() {
    ...
    let seq: Vec<char> = args[1].chars().collect();
    ...
    let keypad = if args.len() > 2 { &num_keypad } else { &dir_keypad };

    let (mut cursor_x, mut cursor_y) = lookup(keypad, 'A');

    // execute instructions
    for c in seq {
        // check for "robot panic"
        // useful to debug.. broken sequences
        if keypad[cursor_x][cursor_y] == '#' {
            println!("...");
            println!("Bad sequence! Illegal instruction {}, goes out of bounds", c);
            break;
        }
        match c {
            'v' => cursor_x += 1,
            ...
            '>' => cursor_y += 1,
            '<' => cursor_y -= 1,
            'A' => print!("{}", keypad[cursor_x][cursor_y]),
            _ => panic!()
        }
    }
