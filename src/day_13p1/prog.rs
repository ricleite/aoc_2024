...
fn solve(a_x: i32, a_y: i32, b_x: i32, b_y: i32, p_x: i32, p_y: i32) -> Option<(i32, i32)> {
...
fn main() {
    ...
    let mut data: Vec<((i32, i32), (i32, i32), (i32, i32))> = Vec::new();
    ...
    loop {
        ...
        let mut a_itr = a_s.split(", ").map(|s| s.split("+").skip(1).next().unwrap().parse::<i32>().unwrap());
        let mut b_itr = b_s.split(", ").map(|s| s.split("+").skip(1).next().unwrap().parse::<i32>().unwrap());
        let mut prize_itr = prize_s.split(", ").map(|s| s.split("=").skip(1).next().unwrap().parse::<i32>().unwrap());

        data.push((
            (a_itr.next().unwrap(), a_itr.next().unwrap()),
            (b_itr.next().unwrap(), b_itr.next().unwrap()),
            (prize_itr.next().unwrap(), prize_itr.next().unwrap())
        ));
