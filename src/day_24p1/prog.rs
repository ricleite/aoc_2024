...
fn main() {
    ...
    let mut wires: BTreeMap<&str, u64> = BTreeMap::from_iter(itr.by_ref().take_while(|s| !s.is_empty())
        .map(|s| {
            let mut itr2 = s.split(": ");
            (itr2.next().unwrap(), itr2.next().unwrap().parse::<u64>().unwrap())
    }));
    ...
    while !ops.is_empty() {
        ...
        for (arg1, op, arg2, res) in &ops {
            ...
            let arg1 = *wires.get(arg1).unwrap();
            let arg2 = *wires.get(arg2).unwrap();
            let val = match *op {
                "AND" => (arg1 != 0 && arg2 != 0) as u64,
                "OR" => (arg1 != 0 || arg2 != 0) as u64,
                "XOR" => ((arg1 != 0) ^ (arg2 != 0)) as u64,
                _ => panic!()
            };

            wires.insert(res, val);
        }
