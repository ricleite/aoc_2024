...
fn main() {
    ...
    for i_do in line.split("do()") {
        let v_donot : Vec<&str> = i_do.split("don't").collect();
        // only the leading element matters, of course - after do(), before donot()
        if v_donot.len() == 0 {
            continue;
        }

        for mul in v_donot[0].split("mul(") {
            ...
            let s1 = v[0];
            if !s1.chars().all(|c| char::is_numeric(c)) {
                continue;
            }
            ...
            let s2 = v2[0];
            if !s2.chars().all(|c| char::is_numeric(c)) {
                continue;
            }
