...
fn main() {
    ...
    let filename = &args[1];
    let iterations = if args.len() <= 2 { 2001 } else { args[2].parse::<usize>().unwrap() };
    let input = read_to_string(filename).unwrap();

    let mut numbers: Vec<u64> = input.lines().map(|s| s.parse::<u64>().unwrap()).collect();
    let mut bananas: Vec<Vec<u32>> = vec![vec![]; numbers.len()];
    let mut bananas_diff: Vec<Vec<i32>> = vec![vec![]; numbers.len()];

    let mut map: HashMap<(i32, i32, i32, i32), u32> = HashMap::new();

    for i in 0..numbers.len() {
        let mut visited: HashSet<(i32, i32, i32, i32)> = HashSet::new();
        for j in 0..iterations {
            let number = &mut numbers[i];
            let digit = number.to_string().chars().last().unwrap().to_digit(10).unwrap();
            bananas[i].push(digit);

            if j > 0 {
                bananas_diff[i].push(digit as i32 - bananas[i][j - 1] as i32);
            }

            if j > 4 {
                let key = (bananas_diff[i][j - 4], bananas_diff[i][j - 3],
                    bananas_diff[i][j - 2], bananas_diff[i][j - 1]);
                ...
                if !visited.contains(&key) {
                    ...
                    let bananas_num = map.get_mut(&key).unwrap();
                    *bananas_num += digit;
                }
            }

            let mul = *number * 64;
            *number = (*number ^ mul) & 0xFFFFFF;
            let div = *number / 32;
            *number = (*number ^ div) & 0xFFFFFF;
            let res = *number * 2048;
            *number = (*number ^ res) & 0xFFFFFF;
        }
    }
