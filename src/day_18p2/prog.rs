...
fn is_blocked(max_x: &usize, max_y: &usize, coords: &Vec<(usize, usize)>, steps: usize) -> bool {
...
fn main() {
    ...
    let idx: Vec<usize> = (1..coords.len()).collect();

    let res = idx.binary_search_by(|idx| {
        let is_blocked_l = is_blocked(&max_x, &max_y, &coords, *idx - 1);
        let is_blocked_r = is_blocked(&max_x, &max_y, &coords, *idx);
        match (is_blocked_l, is_blocked_r) {
