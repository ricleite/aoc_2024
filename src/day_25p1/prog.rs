...
fn main() {
    ...
    loop {
        ...
        let v: Vec<&str> = itr.by_ref().take_while(|s| !s.is_empty()).collect();
        let mut val: Vec<u32> = vec![0; v.first().unwrap().chars().collect::<Vec<char>>().len()];
        for s in &v {
        ...
        // it's a lock
        if header.unwrap().chars().all(|c| c == '#') {
            locks.push(val);
        } else {
            keys.push(val);
        }
