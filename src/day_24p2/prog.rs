...
fn main() {
    ...
    let filename = &args[1];
    let x: Vec<char> = { if args.len() <= 2 { "101010101010101010101010101010101010101010101" } else { &args[2] }}.chars().collect();
    let y: Vec<char> = { if args.len() <= 3 { "010101010101010101010101010101010101010101010" } else { &args[3] }}.chars().collect();

    let input = read_to_string(filename).unwrap();

    let mut itr = input.lines();
    // disgusting, but does the trick
    let mut wires: BTreeMap<&str, (u64, BTreeSet<&str>)> = BTreeMap::from_iter(itr.by_ref().take_while(|s| !s.is_empty())
        .map(|s| {
            let mut itr2 = s.split(": ");
            (itr2.next().unwrap(), (itr2.next().unwrap().parse::<u64>().unwrap(), BTreeSet::new()))
    }));

    // override the values with n1/n2, for debugging
    assert!(x.len() == y.len());
    assert!(wires.get(&format!("x{:02}", x.len())[..]).is_none());
    assert!(wires.get(&format!("y{:02}", y.len())[..]).is_none());
    for idx in 0..x.len() {
        wires.get_mut(&format!("x{:02}", idx)[..]).unwrap().0 = x[idx].to_digit(2).unwrap() as u64;
        wires.get_mut(&format!("y{:02}", idx)[..]).unwrap().0 = y[idx].to_digit(2).unwrap() as u64;
    }
    ...
    for line in itr {
        ...
        let op = v1[1];
        let arg2 = v1[2];
        ops.push((&arg1, &op, &arg2, &res));
    }

    // now, do passes to resolve the operations
    while !ops.is_empty() {
        let mut new_ops: Vec<(&str, &str, &str, &str)> = Vec::new();

        for (arg1, op, arg2, res) in &ops {
            if !wires.contains_key(arg1) || !wires.contains_key(arg2) {
                new_ops.push((arg1, op, arg2, res));
                continue;
            }

            let (n1, s1) = wires.get(arg1).unwrap();
            let (n2, s2) = wires.get(arg2).unwrap();
            let val = match *op {
                "AND" => (*n1 != 0 && *n2 != 0) as u64,
                "OR" => (*n1 != 0 || *n2 != 0) as u64,
                "XOR" => ((*n1 != 0) ^ (*n2 != 0)) as u64,
                _ => panic!()
            };

            let s: BTreeSet<&str> = BTreeSet::from_iter(
                s1.iter().cloned().chain(s2.iter().cloned()).chain([*arg1, *arg2]));
            wires.insert(res, (val, s));
        }
    ...
    for idx in 0..64 {
        let wire = format!("z{:02}", idx);
        if wires.contains_key(&wire[..]) {
            print!("{}", wires.get(&wire[..]).unwrap().0);
        }
    }

    println!("");

    let mut x_in: Vec<char> = Vec::new();
    let mut y_in: Vec<char> = Vec::new();
    let mut z_out: Vec<char> = Vec::new();

    // not brilliant, but the approach here is to:
    // 1) find first bad bit with manipulate x/y
    // 2) look at input, fix it, modify it
    // 3) goto 1)
    let mut bad = false;
    for idx in 0..x.len() {
        let x = format!("x{:02}", idx);
        let y = format!("y{:02}", idx);
        let z = format!("z{:02}", idx);
        let (x1, _) = wires.get(&x[..]).unwrap();
        let (y1, _) = wires.get(&y[..]).unwrap();
        let (z1, z2) = wires.get(&z[..]).unwrap();

        if *x1 + *y1 != *z1 && !bad {
            // it is an adder, so zN needs to use the intermediate registers of zN-1
            // therefore whatever is wrong (especially given the 0101.. + 1010.. input)
            // needs to be in the difference
            bad = true;

            println!("first bad bit: {}", z);
            if idx == 0 {
                println!("candidates: {:?}", z2.iter().chain([&&z[..]]));
            } else {
                let (_, zn) = wires.get(&format!("z{:02}", idx - 1)[..]).unwrap();
                println!("candidates: {:?}", z2.difference(zn).chain([&&z[..]]));
            }
        }

        x_in.append(&mut x1.to_string().chars().collect());
        y_in.append(&mut y1.to_string().chars().collect());
        z_out.append(&mut z1.to_string().chars().collect());
    }

    println!("x: {}", x_in.iter().collect::<String>());
    println!("y: {}", y_in.iter().collect::<String>());
    println!("z: {}", z_out.iter().collect::<String>());
}